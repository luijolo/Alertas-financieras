...
tryCatch({
  ...
  tryCatch({
    ...
    if (!is.na(val_fid)) {
      ...
      prev_raw <- unlist(old_state[["multiplaza_valor"]])
      prev_num <- if (!is.null(prev_raw) && length(prev_raw) > 0) suppressWarnings(as.numeric(as.character(prev_raw[1]))) else NA
  ...
  for (f in fondos_afi) {
    ...
    if (!is.null(res_afi) && status_code(res_afi) == 200) {
      ...
      if (!is.na(val_num)) {
        ...
        prev_num <- if (!is.null(raw_prev) && length(raw_prev) > 0) suppressWarnings(as.numeric(as.character(raw_prev[1]))) else NA
        
        if (is.na(prev_num) || abs(val_num - prev_num) > 1e-4) {
